(function(global) {
  ...
  WEB_UI.isFunction = isFunction;
  WEB_UI.isArray = isArray;

  WEB_UI.bind = function(fn) {
  ...
  WEB_UI.reduce = function(arr, callback, initialValue) {
  ...
  WEB_UI.map = function(arr, fn, thisArg) {
  ...
  WEB_UI.filter = function(arr, fn, thisArg) {
    if (!isArray(arr) || !isFunction(fn)) return arr;
  ...
  WEB_UI.forEach = function(arr, fn, thisArg) {
  ...
  // 函数连续调用
  WEB_UI.chain = function(obj, fn, args) {
